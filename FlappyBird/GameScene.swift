...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupBird() {
        ...
        // 移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width * 1.2 + itemTexture.size().width * 1.8)
        ...
        let createItemAnimation = SKAction.run( {
            ...
            let random_y_range = self.frame.size.height / 4
            // アイテムのY軸の下限
            let apple_lowest_y = UInt32(center_y - itemTexture.size().height / 2 - random_y_range / 2)
            // 1〜random_y_rangeまでのランダムな整数を生成
            let random_y = arc4random_uniform(UInt32(random_y_range))
            // Y軸の下限にランダムな値を足して、アイテムのY座標を決定
            let apple_item_y = CGFloat(apple_lowest_y + random_y)
            
            // アイテムを作成
            let apple = SKSpriteNode(texture: itemTexture)
            apple.position = CGPoint(x: 100, y: apple_item_y * 1.5)
            item.addChild(apple)
            ...
            // スコアアップ用のノード --- ここから ---
            let itemNode = SKNode()
            itemNode.position = CGPoint(x: self.size.width + self.bird.size.width / 4, y: apple.frame.height / 2.0)
            itemNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: apple.size.width, height: self.frame.size.height))
